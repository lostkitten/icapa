...
| --- | --- | --- |
| GIS | SQL Server adapter placeholder | None |
| FactSet | SQL Server adapter placeholder | None |
| ICE Data Indices Library | Library-executor placeholder | None |
| Snowflake | Optional future placeholder | None |
| CSV/Excel | Controlled ad-hoc file input | Local file supplied explicitly |

GIS and FactSet require an injected SQL executor after their connection technology, authentication, schemas, and datasets are defined. ICE Data Indices requires an injected operation executor. Snowflake remains inactive until its account and dataset design are provided. No placeholder contains credentials, connection strings, schemas, queries, or sample production data.
