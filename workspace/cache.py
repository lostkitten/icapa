...
class CachePolicy(str, Enum):
    """Control whether a named workspace may read or replace cached reviews."""
...
class CacheMissError(WorkspaceError):
...
class ArtifactMetadata:
    """Provenance for one immutable artifact."""
...
def canonical_digest(value: Any) -> str:
    """Hash a value after deterministic, JSON-safe normalisation."""

    return sha256(_canonical_json(_canonicalise(value))).hexdigest()
...
def build_run_fingerprint(
    *,
    ...
    methodology_type = _qualified_type_name(methodology)
    payload = {
        "schema_version": WORKSPACE_SCHEMA_VERSION,
        "index_id": index_id,
        "methodology": {
            "type": methodology_type,
            "configuration": _canonicalise(methodology),
        },
        "configuration": _canonicalise(configuration or {}),
        "data_revision": _canonicalise(data_revision),
    }
...
class WorkspaceStore:
    ...
    def __init__(
        ...
        *,
        ...
        self.data_revision = _canonicalise(data_revision)
    ...
    def load_review(self, reference_date, effective_date) -> LoadedReview:
        ...
        reference = _normalise_date(reference_date)
        effective = _normalise_date(effective_date)
    ...
    def save_review(self, artifact: ReviewArtifact) -> ArtifactMetadata:
        ...
        reference = _normalise_date(artifact.reference_date)
        effective = _normalise_date(artifact.effective_date)
        ...
        payload = {
            ...
            "diagnostics": _canonicalise(diagnostics),
        }
    ...
    def record_request(
        ...
        *,
        ...
        request = {
            ...
            "reviews": [
                {
                    "reference_date": _normalise_date(item["reference_date"]).isoformat(),
                    "effective_date": _normalise_date(item["effective_date"]).isoformat(),
    ...
    def _decode_review(
        ...
        stored_reference = _normalise_date(payload.get("reference_date"))
        stored_effective = _normalise_date(payload.get("effective_date"))
...
def _canonicalise(value: Any) -> Any:
    ...
    if is_dataclass(value) and not isinstance(value, type):
        return {
            item.name: _canonicalise(getattr(value, item.name))
