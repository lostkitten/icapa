"""Side-effect-free analytics over completed backtest and simulation results."""
...
class AnalyticsEngine:
    ...
    def analyze(
        ...
        *,
        ...
        formal_turnover_frame = self._formal_turnover(
            formal_turnover, simulation_result, diagnostics
        )
    ...
    def _formal_turnover(
        ...
        source_name = "explicit formal_turnover"
        ...
        if source is None:
            ...
            return pd.DataFrame(columns=["formal_one_way_turnover"])
        ...
        if frame.empty:
            ...
            if "formal_one_way_turnover" not in frame.columns:
                frame["formal_one_way_turnover"] = pd.Series(dtype=float)
            return frame
        ...
        if selected is None:
            raise AnalyticsValidationError(
                f"{source_name} must contain a formal one-way turnover column"
            )
        if selected != "formal_one_way_turnover":
            frame = frame.rename(
                columns={selected: "formal_one_way_turnover"}
            )

        original_turnover = frame["formal_one_way_turnover"]
        ...
        frame["formal_one_way_turnover"] = turnover.clip(lower=0.0)
    ...
    def _performance(
        ...
        if return_columns is not None:
            ...
            selected_pair = next(
                (
                    pair
                    for pair in _RETURN_COLUMN_PAIRS
                    if pair[0] in daily.columns and pair[1] in daily.columns
                ),
                None,
            )
...
def _use_instrument_index(frame: pd.DataFrame, label: str) -> pd.DataFrame:
...
def _validated_weight_series(series: pd.Series, label: str) -> pd.Series:
...
def _normalise_result_weights(weights: pd.DataFrame) -> pd.Series:
    ...
    frame["index_weight"] = _validated_weight_series(
...
def _empty_performance() -> tuple[pd.Series, pd.DataFrame]:
