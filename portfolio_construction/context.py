...
from icapa.data_sources.provenance import ProvenanceRecorder
...
class DataContext:
    ...
    def cons(self) -> pd.DataFrame:
        """Return ``constituents`` through the ``cons`` compatibility name."""
...
def get_constituents(context) -> pd.DataFrame:
    """Return a validated constituent frame from canonical or compatibility APIs."""

    missing = object()
    constituents = getattr(context, "constituents", missing)
    if constituents is missing:
        constituents = getattr(context, "cons", missing)
    if not isinstance(constituents, pd.DataFrame):
        raise TypeError(
            "context must expose constituents as a pandas DataFrame"
        )
    return constituents
...
__all__ = ["DataContainer", "DataContext", "get_constituents"]
